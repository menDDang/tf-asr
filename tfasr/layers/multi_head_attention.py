...
class MultiHeadAttention(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        self.dropout = tf.keras.layers.Dropout(dropout_prob, name="dropout")
    ...
    def build(
        ...
        # input_shape : [query_shape, key_shape, value_shape]
        # query_shape : [B, query_length, query_dim]
        # key_shape : [B, key_length, key_dim]
        # value_shape : [B, value_length, value_dim] 
        query_shape, key_shape, value_shape = input_shape
        query_dim = query_shape[-1]
        key_dim = key_shape[-1]
        value_dim = value_shape[-1]
        output_size = self.output_size if self.output_size is not None else value_dim
        self.query_kernel = self.add_weight(
            name="query_kernel",
            shape=[self.num_heads, query_dim, self.head_size],
        ...
        self.key_kernel = self.add_weight(
            name="key_kernel",
            shape=[self.num_heads, key_dim, self.head_size],
            initializer=self.kernel_initializer,
            regularizer=self.kernel_regularizer,
            constraint=self.kernel_constraint,
        )
        self.value_kernel = self.add_weight(
            name="value_kernel",
            shape=[self.num_heads, value_dim, self.head_size],
            initializer=self.kernel_initializer,
            regularizer=self.kernel_regularizer,
            constraint=self.kernel_constraint,
        )
